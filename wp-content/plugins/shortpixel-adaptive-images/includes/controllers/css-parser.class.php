...
class ShortPixelCssParser {
	...
	//extract src parameters of @font-face
	const REGEX_FONT_FACE = '/@font-face.*{[^}]*src\s*:([^}]+)[;}]/sU';
	//extract url part of src
	const REGEX_FONT_SRC = '/url\((.*)\)/sU';
	...
	public function replace_inline_style_fonts($style) {
		...
		$style = preg_replace_callback(self::REGEX_FONT_FACE, function($matches) {
			...
			$urls = preg_replace_callback(self::REGEX_FONT_SRC, function($matches2) {
				$url = trim($matches2[1],'"\'');
			...
			return  str_replace($matches[1], $urls, $matches[0])  ;
		}, $style);
		return $style;
	}
